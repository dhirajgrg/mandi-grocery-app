...
export const uploadBannerImageToImageKit = async (file) => {
  if (!file) {
    throw new AppError("Please upload a banner image file", 400);
  }
  ...
  const fileExtension = path.extname(file.originalname || "") || ".jpg";
  const baseName = path.basename(
    file.originalname || "banner-image",
    fileExtension,
  );
  const fileName = `${sanitizeFilenamePart(baseName)}-${Date.now()}${fileExtension.toLowerCase()}`;
